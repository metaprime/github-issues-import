...
g = { 'is_dry_run': False } # declare in global scope so that it is accessible everywhere
...
def init_config():
	...
	arg_parser.add_argument('--dry-run', dest='dry_run', action="store_true", help="Do a test run and print the output to the terminal")
	...
	g['is_dry_run'] = args.dry_run # TODO change to config.set('is_dry_run', ...)
...
def import_milestone(source):
	if g['is_dry_run']:
		print("Cannot call import_milestone in a dry run")
		sys.exit(1)
...
def import_label(source):
	if g['is_dry_run'] :
		print("Cannot call import_label in a dry run")
		sys.exit(1)
...
def import_issues(issues):
	...
	for issue in issues:
		...
		body = issue['body']
		body = update_issue_references(body)
